...
async function getRecipients() {
  const resp = await fetch(`${SUPABASE_URL}/rest/v1/alert_recipients?active=eq.true&restaurant_id=eq.${AFRIK_FUSION_ID}&select=email,name`, {
...
async function getComboHours(date) {
  try {
    // Ne pas passer end_date — si start=end, Combo retourne 0 résultat
    const url = `https://partner.combohr.com/api/v1/plannings?start_date=${date}&location_id=c66b876f-fccc-40bf-9743-769be1091257`;
    const resp = await fetch(url, {
      headers: { Authorization: `Bearer NtlQR2cKMITVgAneX3xK5JuWmBZWDX0VSr-2TLne7Sw`, "Content-Type": "application/json" },
    });
    if (!resp.ok) return null;
    const shifts = await resp.json();
    // Filtrer uniquement les shifts de la date demandée
...
function buildEmailHTML(date, ca, monthlyRatio, comboHours, ratioProdHoraire) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const html = buildEmailHTML(targetDate, ca, monthlyRatio, comboHours, ratioProdHoraire);
