...
const ZELTY_API_KEY = "MjA0NzI6dwLg80QXzIUBtNtEe6QdyAuIM3c=";
...
const BELLE_EPINE_ID = "r1775159807169";
...
async function getRestoInfo() {
  const resp = await fetch(`${SUPABASE_URL}/rest/v1/restaurants?select=id,name,objectives,date_overrides&id=eq.${BELLE_EPINE_ID}`, {
    headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
  });
  const rows = await resp.json();
  return rows?.[0] || { id: BELLE_EPINE_ID, name: "Waffle Factory Belle Épine", objectives: {}, date_overrides: {} };
}
...
<div style="max-width:520px;margin:0 auto;background:#fff;border-radius:10px;overflow:hidden;border:1px solid #E2E8F0">
  <div style="background:#F87171;padding:24px 28px">
  ...
  try {
    ...
    const emailResp = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `Rapport CA Waffle Factory Belle Épine – ${dateFR}`,
        html,
      }),
    });
